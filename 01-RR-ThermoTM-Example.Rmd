...
**To see this application, "TMThermoApp <- TRUE" must be entered in the parameters section below.**

```{r}
TMThermoApp <- TRUE

## Other Parameters
    TMThermolocation <- paste0(getwd(),"/Applications/Wells-1894-The-Time-Machine")
...
}else{print("If you wanted to view this application, see start of this section")}
```
...
if(TMThermoApp == TRUE) {
  ...
  p <- ggplot(TMThermoWordFlagdf, aes(y = as.factor(TMThermostemsearchedterm), x = as.numeric(as.character(Lemma_Perc)), color = Category, label = Short_KWIC))
      ...
      ggplotly(pl)
}else{print("If you wanted to view this application, change parameters at the beginning of this section.")}
```
...
}else{print("To view Applications, change parameters at the beginning of this section.")}
...
We can make this even clearer by creating a binned heat map.  First, we add all the values to find the frequency of each category per chapter.
...
}else{print("If you wanted to view this application, change parameters above.")}
```
